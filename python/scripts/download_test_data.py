#!/usr/bin/env python3
"""Download test HDF5/NetCDF4 files from a public S3 bucket to tests/data/.

This is the primary way to get test data for running the test suite.
All test files (HDF5 Group, GDAL, h5py, external, and netcdf-test-data)
are mirrored in S3.

To populate S3 from original sources, use the sync_*.py scripts followed
by upload_test_data.py.
...
Usage:
    ...
    uv run python scripts/download_test_data.py --dry-run

    # Force re-download all files
    uv run python scripts/download_test_data.py --force
"""
...
def main():
    ...
    if not files:
        print("No HDF5/NetCDF4 files found in S3 bucket. Is the bucket populated?")
        sys.exit(1)
